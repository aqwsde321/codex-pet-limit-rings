...
severity: medium
tags: [codex, sqlite, usage-tracking, stop-hook]
related_files:
...
- `response.completed` usage 파서 조건은 최신 DB에서 사용량 행을 잡았으므로, 주된 문제는 JSON 포맷 변경이 아니라 기본 DB 경로 선택이었다.
...
- usage 파서 변경 전에 최신 DB에서 `response.completed` 행이 실제로 누락되는지 먼저 확인한다.
